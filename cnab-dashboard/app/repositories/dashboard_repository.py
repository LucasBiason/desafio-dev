...
class DashboardRepository:
    ...
    def get_transactions_by_type(
        ...
        if has_store_filter:
            sql = f"""
                ...
                LEFT JOIN (
                    SELECT t.id, t.transaction_type_id
                    FROM cnab_transaction t
                    JOIN cnab_store s ON s.id = t.store_id{where}
                ) t ON t.transaction_type_id = tt.id
                GROUP BY tt.id, tt.code, tt.description
                ORDER BY tt.code
            """
        else:
            tx_join_extra = self._join_conditions(clauses)
            sql = f"""
                SELECT
                    tt.description AS type_description,
                    COUNT(t.id) AS transaction_count
                FROM cnab_transaction_type tt
                LEFT JOIN cnab_transaction t ON t.transaction_type_id = tt.id{tx_join_extra}
                GROUP BY tt.id, tt.code, tt.description
                ORDER BY tt.code
            """
        return self._execute_list(sql, params)
    ...
    _DEFAULT_ORDERING = "t.occurred_at DESC, t.occurred_time DESC"

    _DETAIL_SORT_MAP: dict[str, str] = {
        "occurred_at": "t.occurred_at",
        "store_name": "s.name",
        "type_description": "tt.description",
        "amount": "t.amount",
        "owner_name": "s.owner_name",
    }

    def _resolve_ordering(self, ordering: str | None) -> str:
        """Converts an ordering string like '-amount' to a safe SQL ORDER BY clause."""
        if not ordering:
            return self._DEFAULT_ORDERING

        desc = ordering.startswith("-")
        field = ordering.lstrip("-")
        sql_col = self._DETAIL_SORT_MAP.get(field)
        if sql_col is None:
            return self._DEFAULT_ORDERING
    ...
    def get_transactions_detail(
        ...
        rows_sql = f"""
            ...
            JOIN cnab_store s ON s.id = t.store_id
            JOIN cnab_transaction_type tt ON tt.id = t.transaction_type_id{where}
            ORDER BY {self._resolve_ordering(ordering)}
